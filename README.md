...
Needs a fairly fast FreeDOS/DOS machine. It looks not so well optimized but doesn't get faster by disabling all non fancy stuff. Main limiting factor by far seems to be memory bandwidth (and cache). File names are upper case which is DOS default. 
## Typical usage
#include "gl.h"  
